...
const ast = require("safe-eval"); // Alternative to ast.literal_eval
  ...
  try {
    ...
    const imagePart = {
      inlineData: {
        ...
        mimeType: "image/png", // or detect MIME type dynamically
      },
    };
    ...
    // Add these lines to clean the response
    const cleanedText = text
      .replace(/```json/g, "") // Remove opening markdown tags
    ...
    return answers.map((answer) => ({
      ...answer,
      assign: "assign" in answer ? Boolean(answer.assign) : false,
    }));
  } catch (error) {
